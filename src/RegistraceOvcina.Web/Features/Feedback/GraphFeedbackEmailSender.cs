...
/// already used by <c>GraphCharacterPrepEmailSender</c> / <c>InvitationService.SendViaGraphAsync</c>
/// / <c>InboxService.SendNewMessageAsync</c> so we don't fork the outbound pipeline. Only
/// registered when <see cref="MailboxEmailOptions.IsConfigured"/>.
/// </summary>
public sealed class GraphFeedbackEmailSender(
    IHttpClientFactory httpClientFactory,
    IGraphAccessTokenProvider accessTokenProvider,
    IOptions<MailboxEmailOptions> emailOptions,
    ILogger<GraphFeedbackEmailSender> logger) : IFeedbackEmailSender
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!options.IsConfigured)
        {
            throw new InvalidOperationException(
                "Feedback email sending requires Email:SharedMailboxAddress and Graph credentials.");
        }
        ...
        logger.LogInformation(
            "Feedback email sent from {SharedMailbox} to {Recipient}", sharedMailbox, recipientEmail);
    }
}
